...
export class UserService {
  ...
  //  update user
  async updateUser(email: string, updateUserModel: UpdateUserModel): Promise<object> {
    try {
      ...
      let updation = await this.userModel.findOneAndUpdate({ email }, updateUserModel).lean()
      if (!updation) {
  ...
  async login(logInUserModel: LogInUserModel) {
    try {
      ...
      if (!user) {
        this.errorMessageService.addErrorMessage({ message: ErrorMessageKeys.InvalidUser })
      } else {
